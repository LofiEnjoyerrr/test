def sync_manipulations_by_mkb(doctors_ids: Iterable[int]):
    ...
    doctors_to_sync_ids = [doctor.id for doctor in doctors_to_sync]
    doctors_sorted_mkb_types_by_doctor = doctor_sorted_mkb_types_by_doctor(doctors_masters_ids=doctors_to_sync_ids)

    mkb_types = {mkb_type for mkb_types in doctors_sorted_mkb_types_by_doctor.values() for mkb_type in mkb_types}
    manipulations_types_by_mkb_type = manipulation_types_by_mkb_type(mkb_types=mkb_types)

    # Аннотируем к врачам новые типы манипуляций
    for doctor in doctors_to_sync:
        doctor.new_manipulations_types = set()
        for mkb_type in doctors_sorted_mkb_types_by_doctor[doctor.id][:DOCTOR_PRACTICE_MKB_ON_PAGE]:
            doctor.new_manipulations_types.update(manipulations_types_by_mkb_type[mkb_type])
